...
bool Tester::test() {
  ...
  std::shared_ptr<pairs_uav_testing::UAVHandler> uh1;
  std::shared_ptr<pairs_uav_testing::UAVHandler> uh2;
  ...
  if (!uh1->isFlyingNormally() || !uh2->isFlyingNormally()) {
    ...
    ROS_ERROR("[%s]: both UAVs are flying normally, that is wrong", ros::this_node::getName().c_str());
    return false;
  }
}
