...
bool LineTracker::initialize(const ros::NodeHandle &nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                             std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/private/line_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/public/line_tracker.yaml");

  const std::string yaml_prefix = "pairs_uav_trackers/line_tracker/";
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_nh, "LineTracker", _profiler_enabled_);
...
std::tuple<bool, std::string> LineTracker::activate(const std::optional<pairs_msgs::TrackerCommand> &last_tracker_cmd) {
...
std::optional<pairs_msgs::TrackerCommand> LineTracker::update(const pairs_msgs::UavState &                                          uav_state,
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("LineTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
const pairs_msgs::TrackerStatus LineTracker::getStatus() {
...
const std_srvs::TriggerResponse::ConstPtr LineTracker::switchOdometrySource(const pairs_msgs::UavState &new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr LineTracker::setConstraints(const pairs_msgs::DynamicsConstraintsSrvRequest::ConstPtr &cmd) {
...
const pairs_msgs::ReferenceSrvResponse::ConstPtr LineTracker::setReference(const pairs_msgs::ReferenceSrvRequest::ConstPtr &cmd) {
...
const pairs_msgs::VelocityReferenceSrvResponse::ConstPtr LineTracker::setVelocityReference([
...
const pairs_msgs::TrajectoryReferenceSrvResponse::ConstPtr LineTracker::setTrajectoryReference([
...
void LineTracker::mainTimer(const ros::TimerEvent &event) {
  ...
  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("main", _tracker_loop_rate_, 0.01, event);
  pairs_lib::ScopeTimer timer            = pairs_lib::ScopeTimer("LineTracker::main", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
