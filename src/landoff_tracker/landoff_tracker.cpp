...
private:
  ...
  bool callbackTakeoff(pairs_msgs::Vec1::Request& req, pairs_msgs::Vec1::Response& res);
  ...
  pairs_msgs::TrackerCommand position_output_;
...
bool LandoffTracker::initialize(const ros::NodeHandle& nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/private/landoff_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/public/landoff_tracker.yaml");

  const std::string yaml_prefix = "pairs_uav_trackers/landoff_tracker/";
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_nh, "LandoffTracker", _profiler_enabled_);
...
std::tuple<bool, std::string> LandoffTracker::activate([[maybe_unused]] const std::optional<pairs_msgs::TrackerCommand>& last_tracker_cmd) {
...
std::optional<pairs_msgs::TrackerCommand> LandoffTracker::update(const pairs_msgs::UavState&                                           uav_state,
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("LandoffTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
const pairs_msgs::TrackerStatus LandoffTracker::getStatus() {
  ...
  if (idling || (hovering && activate_as_the_first_tracker)) {
    ...
    tracker_status.state = pairs_msgs::TrackerStatus::STATE_TAKEOFF;
    ...
    tracker_status.state = pairs_msgs::TrackerStatus::STATE_LAND;
  }
...
const std_srvs::TriggerResponse::ConstPtr LandoffTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::UavState& new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr LandoffTracker::setConstraints([
    [maybe_unused]] const pairs_msgs::DynamicsConstraintsSrvRequest::ConstPtr& cmd) {


  pairs_lib::set_mutexed(mutex_constraints_, cmd->constraints, constraints_);
...
const pairs_msgs::ReferenceSrvResponse::ConstPtr LandoffTracker::setReference([[maybe_unused]] const pairs_msgs::ReferenceSrvRequest::ConstPtr& cmd) {
...
const pairs_msgs::VelocityReferenceSrvResponse::ConstPtr LandoffTracker::setVelocityReference([
...
const pairs_msgs::TrajectoryReferenceSrvResponse::ConstPtr LandoffTracker::setTrajectoryReference([
...
void LandoffTracker::timerMain(const ros::TimerEvent& event) {
  ...
  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("main", _main_timer_rate_, 0.002, event);
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("LandoffTracker::main", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
bool LandoffTracker::callbackTakeoff(pairs_msgs::Vec1::Request& req, pairs_msgs::Vec1::Response& res) {
