...
#include <pairs_msgs/SpeedTrackerCommand.h>
...
class SpeedTracker : public pairs_uav_managers::Tracker {
public:
...
private:
  ...
  pairs_lib::PublisherHandler<visualization_msgs::MarkerArray> ph_rviz_marker_;
  ...
  pairs_lib::SubscribeHandler<pairs_msgs::SpeedTrackerCommand> sh_command_;

  void callbackCommand(const pairs_msgs::SpeedTrackerCommand::ConstPtr msg);

  // stores the post-processed and transformed command
  pairs_msgs::SpeedTrackerCommand command_;
  ...
  std::future<pairs_msgs::String> switch_tracker_future_;
...
bool SpeedTracker::initialize(const ros::NodeHandle &nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                              std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/private/speed_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/public/speed_tracker.yaml");

  const std::string yaml_prefix = "pairs_uav_trackers/speed_tracker/";
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_nh, "SpeedTracker", _profiler_enabled_);
  ...
  sh_command_ = pairs_lib::SubscribeHandler<pairs_msgs::SpeedTrackerCommand>(shopts, "command", &SpeedTracker::callbackCommand, this);

  // | ----------------------- publishers ----------------------- |

  ph_rviz_marker_ = pairs_lib::PublisherHandler<visualization_msgs::MarkerArray>(nh_, "rviz_marker", 1);

  // | --------------------- service clients -------------------- |

  sch_switch_tracker_ = pairs_lib::ServiceClientHandler<pairs_msgs::String>(common_handlers->parent_nh, "switch_tracker");
...
std::tuple<bool, std::string> SpeedTracker::activate([[maybe_unused]] const std::optional<pairs_msgs::TrackerCommand> &last_tracker_cmd) {
...
std::optional<pairs_msgs::TrackerCommand> SpeedTracker::update(const pairs_msgs::UavState &                                          uav_state,
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("SpeedTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  try {
    uav_heading = pairs_lib::AttitudeConverter(uav_state_.pose.orientation).getHeading();
  }
  catch (...) {
  ...
  if (switch_tracker_called_ && switch_tracker_future_.wait_for(std::chrono::milliseconds(1)) == std::future_status::ready) {
    ...
    switch_tracker_future_ = std::future<pairs_msgs::String>();
  }

  auto command = pairs_lib::get_mutexed(mutex_command_, command_);
...
const pairs_msgs::TrackerStatus SpeedTracker::getStatus() {
...
const std_srvs::TriggerResponse::ConstPtr SpeedTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::UavState &new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr SpeedTracker::setConstraints([
...
const pairs_msgs::ReferenceSrvResponse::ConstPtr SpeedTracker::setReference([[maybe_unused]] const pairs_msgs::ReferenceSrvRequest::ConstPtr &cmd) {
...
const pairs_msgs::VelocityReferenceSrvResponse::ConstPtr SpeedTracker::setVelocityReference([
...
const pairs_msgs::TrajectoryReferenceSrvResponse::ConstPtr SpeedTracker::setTrajectoryReference([
...
void SpeedTracker::callbackCommand(const pairs_msgs::SpeedTrackerCommand::ConstPtr msg) {
  ...
  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("callbackCommand");
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("SpeedTracker::callbackCommand", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);

  pairs_msgs::SpeedTrackerCommandConstPtr external_command = msg;
  ...
  pairs_msgs::SpeedTrackerCommand transformed_command = *external_command;

  auto old_command = pairs_lib::get_mutexed(mutex_command_, command_);
  ...
  if (transformed_command.use_heading) {
    ...
    pairs_msgs::ReferenceStamped temp_ref;
  ...
  if (!is_active_) {
    ...
    try {
      transformed_command.heading = pairs_lib::AttitudeConverter(uav_state.pose.orientation).getHeading();
    }
    catch (...) {
      return;
    }
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_trackers::speed_tracker::SpeedTracker, pairs_uav_managers::Tracker)
