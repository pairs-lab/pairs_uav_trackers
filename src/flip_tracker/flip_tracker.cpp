...
#include <pairs_uav_trackers/flip_trackerConfig.h>
...
private:
  ...
  pairs_msgs::TrackerCommand activation_cmd_;
  ...
  void dynamicReconfigureCallback(pairs_uav_trackers::flip_trackerConfig &config, uint32_t level);

  boost::recursive_mutex                       config_mutex_;
  typedef pairs_uav_trackers::flip_trackerConfig Config;
  typedef dynamic_reconfigure::Server<Config>  ReconfigureServer;
  boost::shared_ptr<ReconfigureServer>         reconfigure_server_;
  pairs_uav_trackers::flip_trackerConfig         drs_params_;
...
bool FlipTracker::initialize(const ros::NodeHandle &nh, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                             std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/private/flip_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ros::package::getPath("pairs_uav_trackers") + "/config/public/flip_tracker.yaml");

  const std::string yaml_prefix = "pairs_uav_trackers/flip_tracker/";
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_nh, "FlipTracker", _profiler_enabled_);
...
std::tuple<bool, std::string> FlipTracker::activate(const std::optional<pairs_msgs::TrackerCommand> &last_tracker_cmd) {
...
std::optional<pairs_msgs::TrackerCommand> FlipTracker::update(const pairs_msgs::UavState &                                          uav_state,
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("FlipTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  pairs_msgs::TrackerCommand tracker_cmd = activation_cmd_;
  ...
  double hover_throttle = pairs_lib::quadratic_throttle_model::forceToThrottle(common_handlers_->throttle_model, mass * g);
...
const pairs_msgs::TrackerStatus FlipTracker::getStatus() {
...
const std_srvs::TriggerResponse::ConstPtr FlipTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::UavState &new_uav_state) {
...
const pairs_msgs::DynamicsConstraintsSrvResponse::ConstPtr FlipTracker::setConstraints([
...
const pairs_msgs::ReferenceSrvResponse::ConstPtr FlipTracker::setReference([[maybe_unused]] const pairs_msgs::ReferenceSrvRequest::ConstPtr &cmd) {
...
const pairs_msgs::VelocityReferenceSrvResponse::ConstPtr FlipTracker::setVelocityReference([
...
const pairs_msgs::TrajectoryReferenceSrvResponse::ConstPtr FlipTracker::setTrajectoryReference([
...
void FlipTracker::dynamicReconfigureCallback(pairs_uav_trackers::flip_trackerConfig &config, [[maybe_unused]] uint32_t level) {
