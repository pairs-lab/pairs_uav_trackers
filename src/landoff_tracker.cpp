...
class LandoffTracker : public pairs_uav_managers::Tracker {
public:
...
private:
  ...
  pairs_uav_managers::Controller::ControlOutput last_control_output_;
  ...
  pairs_lib::ServiceServerHandler<pairs_msgs::srv::Vec1>    ss_takeoff_;
  pairs_lib::ServiceServerHandler<std_srvs::srv::Trigger> ss_land_;
  pairs_lib::ServiceServerHandler<std_srvs::srv::Trigger> ss_eland_;

  void callbackTakeoff(const std::shared_ptr<pairs_msgs::srv::Vec1::Request> request, std::shared_ptr<pairs_msgs::srv::Vec1::Response> response);
  ...
  pairs_msgs::msg::TrackerCommand position_output_;
...
bool LandoffTracker::initialize(const rclcpp::Node::SharedPtr &node, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                                std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  pairs_lib::ParamLoader param_loader_parent(common_handlers->parent_node, "LandoffTracker");
  ...
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_trackers") + "/config/private/landoff_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_trackers") + "/config/public/landoff_tracker.yaml");
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_node, "LandoffTracker", _profiler_enabled_);

  // | ------------------------ services ------------------------ |

  ss_takeoff_ = pairs_lib::ServiceServerHandler<pairs_msgs::srv::Vec1>(
  ...
  ss_land_  = pairs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(node_, "~/" + private_handlers_->name_space + "/land",
                                                                    std::bind(&LandoffTracker::callbackLand, this, std::placeholders::_1, std::placeholders::_2),
                                                                    rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_eland_ = pairs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
...
std::tuple<bool, std::string> LandoffTracker::activate([[maybe_unused]] const std::optional<pairs_msgs::msg::TrackerCommand> &last_tracker_cmd) {
...
std::optional<pairs_msgs::msg::TrackerCommand> LandoffTracker::update(const pairs_msgs::msg::UavState                                      &uav_state,
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer(node_, "LandoffTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
const pairs_msgs::msg::TrackerStatus LandoffTracker::getStatus() {
  ...
  if (idling || (hovering && activate_as_the_first_tracker)) {
    ...
    tracker_status.state = pairs_msgs::msg::TrackerStatus::STATE_TAKEOFF;
    ...
    tracker_status.state = pairs_msgs::msg::TrackerStatus::STATE_LAND;
  }
...
const std::shared_ptr<std_srvs::srv::Trigger::Response> LandoffTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::msg::UavState &new_uav_state) {
...
LandoffTracker::hover([[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request> &request) {
  ...
  auto [current_horizontal_speed, current_vertical_speed, current_heading, current_vertical_direction] =
      pairs_lib::get_mutexed(mutex_state_, current_horizontal_speed_, current_vertical_speed_, current_heading_, current_vertical_direction_);
...
const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Response>
LandoffTracker::setConstraints([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Request> &request) {
...
LandoffTracker::setReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::ReferenceSrv::Request> &request) {
...
const std::shared_ptr<pairs_msgs::srv::VelocityReferenceSrv::Response>
LandoffTracker::setVelocityReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::VelocityReferenceSrv::Request> &request) {
...
const std::shared_ptr<pairs_msgs::srv::TrajectoryReferenceSrv::Response>
LandoffTracker::setTrajectoryReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::TrajectoryReferenceSrv::Request> &request) {
...
void LandoffTracker::accelerateVertical(void) {
  ...
  // copy member variables
  auto [current_vertical_speed, state_z] = pairs_lib::get_mutexed(mutex_state_, current_vertical_speed_, state_z_);
  auto goal_z                            = pairs_lib::get_mutexed(mutex_goal_, goal_z_);
  ...
  // set the right vertical direction
  {
    ...
    current_vertical_direction_ = pairs_lib::signum(tar_z);
  }

  auto current_vertical_direction = pairs_lib::get_mutexed(mutex_state_, current_vertical_direction_);
...
void LandoffTracker::decelerateVertical(void) {
  ...
  auto current_vertical_speed = pairs_lib::get_mutexed(mutex_state_, current_vertical_speed_);
...
void LandoffTracker::stopHorizontal(void) {

  {
    ...
    if (std::abs(dist_x / dt) > 1.0) {
      dist_x = pairs_lib::signum(dist_x) * (1.0 * dt);
    }

    if (std::abs(dist_y / dt) > 1.0) {
      dist_y = pairs_lib::signum(dist_y) * (1.0 * dt);
    }
...
void LandoffTracker::stopVertical(void) {

  {
    ...
    if (std::abs(dist_z / dt) > 1.0) {
      dist_z = pairs_lib::signum(dist_z) * (1.0 * dt);
    }
...
void LandoffTracker::timerMain() {
  ...
  // copy member variables
  auto uav_state = pairs_lib::get_mutexed(mutex_uav_state_, uav_state_);
  auto [state_x, state_y, state_z, current_horizontal_speed, current_vertical_speed, current_heading, current_vertical_direction] = pairs_lib::get_mutexed(
  ...
  auto [goal_x, goal_y, goal_z] = pairs_lib::get_mutexed(mutex_goal_, goal_x_, goal_y_, goal_z_);
  auto last_control_output      = pairs_lib::get_mutexed(mutex_last_control_output_, last_control_output_);
  ...
  pairs_lib::Routine    profiler_routine = profiler_.createRoutine("main");
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer(node_, "LandoffTracker::main", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  if (taking_off_) {
    ...
    if (error_size > _max_position_difference_) {
      ...
      if (pairs_lib::geometry::dist(vec3_t(future_state_x, future_state_y, future_state_z), vec3_t(uav_x, uav_y, uav_z)) > error_size) {
...
void LandoffTracker::callbackTakeoff(const std::shared_ptr<pairs_msgs::srv::Vec1::Request> request, std::shared_ptr<pairs_msgs::srv::Vec1::Response> response) {
  ...
  double uav_heading = pairs_lib::AttitudeConverter(uav_state.pose.orientation).getHeading();
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_trackers::landoff_tracker::LandoffTracker, pairs_uav_managers::Tracker)
