...
class FlipTracker : public pairs_uav_managers::Tracker {
public:
...
private:
  ...
  pairs_lib::ServiceServerHandler<std_srvs::srv::Trigger> ss_flip_;
  ...
  pairs_msgs::msg::TrackerCommand activation_cmd_;
...
bool FlipTracker::initialize(const rclcpp::Node::SharedPtr &node, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                             std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  pairs_lib::ParamLoader param_loader_parent(common_handlers->parent_node, "FlipTracker");
  ...
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_trackers") + "/config/private/flip_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_trackers") + "/config/public/flip_tracker.yaml");
  ...
  dynparam_mgr_->register_param("phases/z_acceleration/mode", &drs_params_.z_mode, pairs_lib::DynparamMgr::range_t<int>(0, 2));
  dynparam_mgr_->register_param("phases/z_acceleration/acceleration", &drs_params_.z_acceleration, pairs_lib::DynparamMgr::range_t<double>(0, 10.0));
  dynparam_mgr_->register_param("phases/z_acceleration/throttle", &drs_params_.z_throttle, pairs_lib::DynparamMgr::range_t<double>(0, 1.0));
  dynparam_mgr_->register_param("phases/z_acceleration/velocity_gain_from_rot", &drs_params_.velocity_gain_from_rot,
                                pairs_lib::DynparamMgr::range_t<double>(0, 2.0));
  dynparam_mgr_->register_param("phases/flipping_pulse/attitude_rate", &drs_params_.attitude_rate, pairs_lib::DynparamMgr::range_t<double>(0, 30.0));
  dynparam_mgr_->register_param("phases/flipping_pulse/axis", &drs_params_.axis, pairs_lib::DynparamMgr::range_t<int>(0, 1));
  dynparam_mgr_->register_param("phases/flipping_pulse/direction", &drs_params_.direction, pairs_lib::DynparamMgr::range_t<int>(0, 1));
  ...
  ss_flip_ = pairs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(node_, "~/" + private_handlers_->name_space + "/flip",
                                                                   std::bind(&FlipTracker::callbackFlip, this, std::placeholders::_1, std::placeholders::_2),
                                                                   cbkgrp_ss_);

  // | ------------------------ profiler ------------------------ |

  profiler_ = pairs_lib::Profiler(common_handlers->parent_node, "FlipTracker", _profiler_enabled_);
...
std::tuple<bool, std::string> FlipTracker::activate(const std::optional<pairs_msgs::msg::TrackerCommand> &last_tracker_cmd) {
  ...
  pairs_lib::set_mutexed(mutex_current_state_, STATE_IDLE, current_state_);
...
std::optional<pairs_msgs::msg::TrackerCommand> FlipTracker::update(const pairs_msgs::msg::UavState                                      &uav_state,
                                                                 [[maybe_unused]] const pairs_uav_managers::Controller::ControlOutput &last_control_output) {

  auto current_state = pairs_lib::get_mutexed(mutex_current_state_, current_state_);
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer(node_, "FlipTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
  ...
  pairs_msgs::msg::TrackerCommand tracker_cmd = activation_cmd_;

  tracker_cmd.header.stamp = clock_->now();

  // rotate the drone's z axis
  tf2::Transform uav_state_transform = pairs_lib::AttitudeConverter(uav_state.pose.orientation);
  ...
  double hover_throttle = pairs_lib::quadratic_throttle_model::forceToThrottle(common_handlers_->throttle_model, mass * g, *node_);
  ...
  case STATE_ACCELERATION: {
    ...
    if ((clock_->now() - state_change_time_).seconds() >= 2 * z_acceleration_duration_) {
      ...
      pairs_lib::set_mutexed(mutex_current_state_, STATE_RECOVERY, current_state_);
    ...
    if (uav_state.velocity.linear.z > 0.95 * z_vel_gained_by_flipping_) {
      ...
      pairs_lib::set_mutexed(mutex_current_state_, STATE_FLIPPING_PULSE, current_state_);
      state_change_time_ = clock_->now();
    }
  ...
  case STATE_FLIPPING_PULSE: {
    ...
    if ((clock_->now() - state_change_time_).seconds() >= _pulse_timeout_) {
      ...
      pairs_lib::set_mutexed(mutex_current_state_, STATE_FLIPPING_INTERTIA, current_state_);
...
const pairs_msgs::msg::TrackerStatus FlipTracker::getStatus() {
...
const std::shared_ptr<std_srvs::srv::Trigger::Response> FlipTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::msg::UavState &new_uav_state) {
...
const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Response>
FlipTracker::setConstraints([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Request> &request) {
...
FlipTracker::setReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::ReferenceSrv::Request> &request) {
...
const std::shared_ptr<pairs_msgs::srv::VelocityReferenceSrv::Response>
FlipTracker::setVelocityReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::VelocityReferenceSrv::Request> &request) {
...
const std::shared_ptr<pairs_msgs::srv::TrajectoryReferenceSrv::Response>
FlipTracker::setTrajectoryReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::TrajectoryReferenceSrv::Request> &request) {
...
bool FlipTracker::callbackFlip([[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  ...
  if (drs_params.z_mode == 0) {
    ...
    z_acceleration_acc_ = (pairs_lib::quadratic_throttle_model::throttleToForce(common_handlers_->throttle_model, drs_params.z_throttle) / mass) - g;
  ...
  try {
    initial_heading_ = pairs_lib::AttitudeConverter(uav_state.pose.orientation).getHeading();
  }
  catch (...) {
  ...
  pairs_lib::set_mutexed(mutex_current_state_, STATE_ACCELERATION, current_state_);
...
bool FlipTracker::checkState(void) {
  ...
  try {
    heading_rate = pairs_lib::AttitudeConverter(uav_state_.pose.orientation).getHeadingRate(uav_state_.velocity.angular);
  }
  catch (...) {
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_trackers::flip_tracker::FlipTracker, pairs_uav_managers::Tracker)
