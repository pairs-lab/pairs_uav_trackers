...
class LineTracker : public pairs_uav_managers::Tracker {
public:
...
bool LineTracker::initialize(const rclcpp::Node::SharedPtr &node, std::shared_ptr<pairs_uav_managers::control_manager::CommonHandlers_t> common_handlers,
                             std::shared_ptr<pairs_uav_managers::control_manager::PrivateHandlers_t> private_handlers) {
  ...
  pairs_lib::ParamLoader param_loader_parent(common_handlers->parent_node, "LineTracker");
  ...
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_trackers") + "/config/private/line_tracker.yaml");
  private_handlers->param_loader->addYamlFile(ament_index_cpp::get_package_share_directory("pairs_uav_trackers") + "/config/public/line_tracker.yaml");
  ...
  profiler_ = pairs_lib::Profiler(common_handlers->parent_node, "LineTracker", _profiler_enabled_);
...
std::tuple<bool, std::string> LineTracker::activate(const std::optional<pairs_msgs::msg::TrackerCommand> &last_tracker_cmd) {
...

  {
    ...
    if (last_tracker_cmd) {
      ...
      if (last_tracker_cmd->use_heading) {
        ...
        try {
          state_heading_ = pairs_lib::AttitudeConverter(last_tracker_cmd->orientation).getHeading();
        }
        catch (...) {
...
std::optional<pairs_msgs::msg::TrackerCommand> LineTracker::update(const pairs_msgs::msg::UavState                                      &uav_state,
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer(node_, "LineTracker::update", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);
...
const pairs_msgs::msg::TrackerStatus LineTracker::getStatus() {
...
const std::shared_ptr<std_srvs::srv::Trigger::Response> LineTracker::switchOdometrySource([[maybe_unused]] const pairs_msgs::msg::UavState &new_uav_state) {
...
const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Response>
LineTracker::setConstraints([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::DynamicsConstraintsSrv::Request> &request) {
...
LineTracker::setReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::ReferenceSrv::Request> &request) {

  std::shared_ptr<pairs_msgs::srv::ReferenceSrv::Response> response = std::make_shared<pairs_msgs::srv::ReferenceSrv::Response>();

  auto state_heading = pairs_lib::get_mutexed(mutex_state_, state_heading_);
...
const std::shared_ptr<pairs_msgs::srv::VelocityReferenceSrv::Response>
LineTracker::setVelocityReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::VelocityReferenceSrv::Request> &request) {
...
const std::shared_ptr<pairs_msgs::srv::TrajectoryReferenceSrv::Response>
LineTracker::setTrajectoryReference([[maybe_unused]] const std::shared_ptr<pairs_msgs::srv::TrajectoryReferenceSrv::Request> &request) {
...
void LineTracker::accelerateHorizontal(void) {
  ...
  // copy member variables
  auto [goal_x, goal_y]                             = pairs_lib::get_mutexed(mutex_goal_, goal_x_, goal_y_);
  auto [state_x, state_y, current_horizontal_speed] = pairs_lib::get_mutexed(mutex_state_, state_x_, state_y_, current_horizontal_speed_);
  ...
  auto current_heading = pairs_lib::get_mutexed(mutex_state_, current_heading_);
...
void LineTracker::accelerateVertical(void) {
  ...
  auto [state_z, current_vertical_speed] = pairs_lib::get_mutexed(mutex_state_, state_z_, current_vertical_speed_);
...
void LineTracker::decelerateHorizontal(void) {
  ...
  auto current_horizontal_speed = pairs_lib::get_mutexed(mutex_state_, current_horizontal_speed_);
...
void LineTracker::timerMain() {
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer(node_, "LineTracker::main", common_handlers_->scope_timer.logger, common_handlers_->scope_timer.enabled);

  auto [goal_x, goal_y, goal_z]    = pairs_lib::get_mutexed(mutex_goal_, goal_x_, goal_y_, goal_z_);
  auto [state_x, state_y, state_z] = pairs_lib::get_mutexed(mutex_state_, state_x_, state_y_, state_z_);
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_trackers::line_tracker::LineTracker, pairs_uav_managers::Tracker)
